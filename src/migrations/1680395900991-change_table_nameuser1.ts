...
export class changeTableNameUser1680395900991 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "name" ALTER COLUMN "updated_at" DROP DEFAULT`
    );
    await queryRunner.query(
      `ALTER TABLE "name" ALTER COLUMN "created_at" DROP DEFAULT`
    );
    await queryRunner.query(
      `ALTER TABLE "name" ALTER COLUMN "uuid" DROP DEFAULT`
    );
    await queryRunner.query(
      `ALTER TABLE "name" DROP CONSTRAINT "PK_c49f64b685ea46b08143674c6d5"`
    );
